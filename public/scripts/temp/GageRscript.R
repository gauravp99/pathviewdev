...
#pvwdir = readLines(paste(getwd(),"/data/publicPath.txt",sep=""))
...
if(args2$data.type=="gene")
            pv.out.list <- sapply(outnames[ii], function(pid) pathview(gene.data = exprs.d, pathway.id = pid, species = species, kegg.dir=kegg.dir, gene.idtype=gid.type))
        else if(args2$data.type=="compound") pv.out.list <- sapply(outnames[ii], function(pid) pathview(cpd.data = exprs.d, pathway.id = pid, species = species, kegg.dir=kegg.dir, cpd.idtype=gid.type))
