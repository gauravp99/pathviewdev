...
class gageAnalysisController extends Controller {
	...
	public function index($analysis)
	{
		...
		if (strcmp($analysis, 'newAnalysis') == 0) {
			...
			else if(strcmp($analysis, 'exampleGageAnalysis3') == 0){
				$file_location1 = Config::get('constants.example3_gage_file_location');
				$filename = Config::get('constants.example3_gage_file_name');
			}
		...
		if (isset($_POST['geneIdType'])) {
			...
			$runHashdata['anal_type'] = $analysis;
			if (Auth::user())
			...
			Redis::set($unique_analysis_id,json_encode($runHashdata));
			Redis::set($unique_analysis_id.":Status","false");
			...
			if(!$queueEnabled){
				...
				if (intval($jobs) >= 2){
					...
					$queued_job = new QueueJob($unique_analysis_id,'gage');
					array_push($wjobids,$queued_job);
					Redis::set("wids:".$this->uID,json_encode($wjobids));
					//flag to check the wait status of this analysis
					Redis::set("wait:".$unique_analysis_id, "false");

					$process_queue_id = -1;


					return view('gage_pages.analysis.QueueResult')->with(array('exception' => null,
						'directory' => public_path() . "/" . $destFile,
						'directory1' => $destFile,
					...
					$process_queue_id = Queue::push(new RunQueuedJob($unique_analysis_id, $this->uID,'gage'));
...
public function discreteGageAnalysis()
	...
	foreach ($_POST['geneSet'] as $geneSet) {
	 ...
	 if (isset($_POST['dataType'])) {
	    ...
	    $argument .= "mid.type:" . $_POST['geneIdType'] . ";";
            ...
            if (strcmp($_POST['geneIdType'], 'custom') == 0) {
                if(Input::hasFile('geneIdFile'))
                {
                    ...
                    $file1 = public_path() . "/" . "data/c1_all_v3_0_symbols.gmt";
		...
		$runHashdata['anal_type'] = "discreteGageAnalysis";
		if (Auth::user())
			$runHashdata['user'] = Auth::user()->id;
		else
			$runHashdata['user'] = "demo";
...
public function newGageAnalysis()
	...
	public function ExampleGageAnalysis3()
	{

		$d = new GageAnalysisController();
		return $d->index("exampleGageAnalysis3");
